...
class CoverageFilter(object):
    ...
    def __init__(self, *cover_targets):
        ...
        Arguments:
            cover_targets: a list of symbols to cover
        """
        self._old_trace = None  # Reference to the previous fuction
        self.is_tracing = False  # Flag to indicate it's our turn.

        # A stack tracking whether or not our filter is engaged
        self.is_covering = [False]

        # A list of file (and symbols per file) to include in the filter
        self.files = {}
        for target in cover_targets:
            ...
            if len(parts) == 2:
                ...
                filename = parts[0]
                symbol = '*'  # use askerik as indicator we accept all symbols
    ...
    def _trace(self, frame, event, arg_unused):
        ...
        if self.is_covering[-1]:
            ...
            if event == 'call' and sys.gettrace() != self._trace:
                # The coverage CTracer resets the trace function every time a
                # call event is seen. There's no way around this, so we'll
                # just have to undo that.
                sys.settrace(self._trace)
